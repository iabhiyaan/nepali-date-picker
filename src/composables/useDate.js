...
export default function useDate(props) {
  ...
  const formattedYear = computed(() =>
    formatNepali.value ? date.value.format("yyyy") : date.value.format("YYYY"),
  );
  ...
  const formattedDate = computed(() =>
    formatNepali.value
      ? date.value.format(defaultDateFormat.value)
      : date.value.format(defaultEnglishDateFormat.value),
  );

  const formattedTodayText = computed(() =>
    formatNepali.value ? "आज" : "Today",
  );
  ...
  const getMonthsList = computed(() =>
    formatNepali.value ? NEPALI_MONTH : ENGLISH_NEPALI_MONTH,
  );
  ...
  const weekdays = computed(() =>
    formatNepali.value ? NEPALI_WEEK : ENGLISH_WEEK,
  );
